...
export default async function AdminProductsPage() {
  ...
  return <AdminShell user={user}>
    <header className="admin-content-header"><div><p>Catalogue</p><h1>Les parfums.</h1></div><Link className="admin-create-button" href="/admin/produits/nouveau"><Plus /> Nouveau produit</Link></header>
    <div className="admin-product-list">
      ...
      {products.length === 0 ? <div className="admin-empty">Aucun parfum. La page blanche, mais avec un meilleur sillage.</div> : products.map((product) => {
        ...
        return <article className="admin-product-row" key={product.id}>
          <div className="admin-product-identity"><div className="admin-product-thumb">{product.images[0] && <Image src={product.images[0]} alt="" width={64} height={80} />}</div><div><Link href={`/admin/produits/${product.id}`}>{product.name}</Link><small>{startingPrice === undefined ? "Sans prix" : `Dès ${formatPrice(startingPrice)}`} · /{product.slug}</small></div></div>
